 ...
 * the upstream registry PR swaps them for `import type { Plugin, Action, … }
 * from '@elizaos/core'` (shape-compatible).
 *
 * Optional env: WEALTHVILLE_API_KEY, WEALTHVILLE_API_URL.
 */

// ── Minimal structural types matching @elizaos/core ─────────────────────────
type Runtime = Record<string, unknown>;
interface Memory { content: { text?: string; [k: string]: unknown }; [k: string]: unknown }
type HandlerCallback = (response: { text: string; [k: string]: unknown }) => Promise<unknown> | unknown;
export interface Action {
    name: string;
    similes: string[];
    description: string;
    examples: Array<Array<{ user: string; content: { text: string; action?: string } }>>;
    validate: (runtime: Runtime, message: Memory) => Promise<boolean>;
    handler: (runtime: Runtime, message: Memory, state?: unknown, options?: unknown, callback?: HandlerCallback) => Promise<boolean>;
}
...
const getPoolScore: Action = {
    ...
    examples: [[
        { user: '{{user1}}', content: { text: 'Is Czfq3xZZDmsdGdUyrNLtRhGc47cXcZtLG4crryfu44zE safe to LP?' } },
        { user: '{{agent}}', content: { text: 'SOL/USDC (orca-whirlpool): verdict ENTER, Wealthville Score 93/100.', action: 'GET_POOL_SCORE' } },
    ]],
    ...
    handler: async (_rt, message, _state, _opts, callback) => {
        ...
        try {
            ...
            await callback?.({ text: fmtScore(d) });
            return true;
        } catch (e: any) {
            await callback?.({ text: `Couldn't fetch a Wealthville score for that pool (${e.message}). It may not be scored yet.` });
            return false;
        }
    },
};
...
const getTopPools: Action = {
    ...
    examples: [[
        { user: '{{user1}}', content: { text: 'What are the best Solana pools to LP right now?' } },
        { user: '{{agent}}', content: { text: 'Top pools by Wealthville Score: SOL/USDC (meteora-dlmm) 94, SOL/USDC (orca-whirlpool) 94, …', action: 'GET_TOP_POOLS' } },
    ]],
    ...
    handler: async (_rt, _message, _state, _opts, callback) => {
        try {
            ...
            await callback?.({ text: `Top pools by Wealthville Score:\n${lines.join('\n')}\n${DISCLAIMER}` });
            return true;
        } catch (e: any) {
            await callback?.({ text: `Couldn't fetch top pools right now (${e.message}).` });
            return false;
        }
    },
};
...
const getTrackRecord: Action = {
    ...
    examples: [[
        { user: '{{user1}}', content: { text: 'How accurate are these Wealthville scores anyway?' } },
        { user: '{{agent}}', content: { text: 'Over the last 30 days: ENTER hit rate 59.9% across 3,817 resolved signals…', action: 'GET_WEALTHVILLE_TRACK_RECORD' } },
    ]],
    ...
    handler: async (_rt, _message, _state, _opts, callback) => {
        try {
            ...
            await callback?.({ text: `Wealthville live track record, last ${d.window_days} days (misses included, ledger immutable):\n${lines.join('\n')}\nFull ledger: https://www.wealthville.net/track-record` });
            return true;
        } catch (e: any) {
            await callback?.({ text: `Couldn't fetch the track record right now (${e.message}).` });
            return false;
        }
    },
};
